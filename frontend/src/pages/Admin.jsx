          ...
          <div key={order._id} className="rounded-2xl border border-rose-100 bg-white p-4">
            ...
            <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="rounded-xl border border-rose-100 p-3 bg-rose-50 md:col-span-2">
                <p className="text-xs uppercase tracking-wider text-slate-500 mb-2">Customer Shipping Address</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-slate-700">
                  <div>
                    <p className="font-semibold text-slate-800">{order.customerAddress?.fullName || order.customerName || 'Not set'}</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3 text-sm text-slate-700">
                  <div>
                  ...
                  <div className="rounded-xl border border-white bg-white p-3">
                    ...
                    <p className="font-semibold text-slate-800">{order.deliveryPartner || 'Fixed Shipping'}</p>
                    <p className="text-xs text-slate-600 mt-1">{order.deliveryCharge > 0 ? toCurrency(order.deliveryCharge) : 'FREE'}</p>
                  </div>
                </div>
                <p className="text-xs text-slate-600 mt-2">Accepted orders only need a dispatch date now. Shipping charges are fixed-rate and not courier-calculated.</p>
              </div>
                ...
                return (
                  <div key={`${order._id}-${index}`} className="rounded-xl border border-rose-100 p-3 flex items-center gap-3 bg-rose-50">
                    {itemImage ? (
                      ...
                      <div className="h-12 w-12 rounded-lg bg-rose-100 text-rose-500 flex items-center justify-center flex-shrink-0">
                        <Boxes size={18} />
                      </div>
                    )}
                    <div>
